...
- **Branch names must be relevant** to the task (e.g., `feat/stats-command`, `fix/subagent-matching`).
- **Subagents can commit** on the working branch as they go — small, atomic commits are encouraged.
- **Only the main agent merges and pushes.** Subagents do not merge to main or push to remote.
- **Commit messages** should be concise, imperative, and describe the "why" not the "what".

## Build & Run

- Python 3.10+, managed via `uv`
- Dependencies declared in `pyproject.toml` inline script metadata or `[project.dependencies]`
- Run: `uv run trawl` (after pyproject.toml is set up)
- Test: `uv run pytest`
